...
public class ManoJugador : MonoBehaviour
{
	...
	public void HabilitarCartasParaSeleccionDinamita(int maxSeleccion)
	{
		...
		Debug.Log($"?? Habilitada selección de {maxSeleccion} cartas para descarte INMEDIATO");
	...
	public void ProcesarClicCartaDinamita(GameObject carta)
	{
		...
		{
			Debug.Log($"?? Carta ya está siendo procesada: {carta.name}");
			return;
		}

		// Agregar temporalmente a la selección para evitar doble clic
	...
	// NUEVO: Método para descarte inmediato de una carta
	private void IniciarDescarteInmediato(GameObject carta)
	{
		...
		{
			// Fallback: remover de la selección si no se puede procesar
